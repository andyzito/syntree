...
function strip_html($html) {
	...
	foreach(preg_split("/((\r?\n)|(\r\n?))/", $html) as $line){
		if (!preg_match('/<!DOCTYPE|<head>|<meta|<html|<\/html|<title|<body>|<\/head>|<\/body>/', $line)) {
			$line = preg_replace('/<!--.*/', '<p class="own_requirement">Blue requirements are those I added on my own, while all other requirements were derived from conversations with \'clients\'.', $line);
